...
class user
{
     ...
     * @param string $author_name author name of the use
    ...
    {
        $data = $this->dbconn->getUserData($this->id);
        $this->role = $data["role"];
        $this->name = $data["name"];
    ...
    {
        $this->dbconn->updateUserData($this->id, $role, $name, $this->pass_hash, $author_name, $email);
        $this->update();
    }
    /**
    ...
    public function setPassword(string $password)
    {
        ...
        $this->dbconn->updateUserData($this->id, $this->role, $this->name, $hash, $this->author_name, $this->email);
    }
    public function delete()
    {
        $this->dbconn->deleteUserData($this->id);
    }
}
...
class userManager
{
    ...
    {
        $data = $this->dbconn->getUserData($id);
        return new user(
            $data["id"],
            $data["role"],
            $data["name"],
    ...
    {
        $data = $this->dbconn->getAllUserData();
        $users = array();
        foreach ($data as $userData) {
            $user = new user($userData["id"], $userData["role"], $userData["name"], $userData["pass_hash"], $userData["author_name"], $userData["email"], $dbconn);
     ...
     */
    public function createNewUser(int $role, string $name, string $password, string $author_name, string $email)
    {
        $hash = password_hash($password, PASSWORD_BCRYPT);
        $data = $this->dbconn->addUserData($role, $name, $hash, $author_name, $email);
        return new user(
            $data["id"],
            $data["role"],
            $data["name"],
